...
struct HomeScreenRoomCell: View {
    ...
    @Environment(\.synThemePalette) private var synTheme
    ...
    var body: some View {
        ...
        .buttonStyle(HomeScreenRoomCellButtonStyle(isSelected: isSelected, palette: synTheme))
        ...
        if dynamicTypeSize < .accessibility3 {
            RoomAvatarImage(avatar: room.avatar,
                ...
                .overlay(alignment: .bottomTrailing) {
                    if room.isDirect {
                        Circle()
                            .fill(room.presence?.presence.color ?? .gray)
                            .frame(width: 13, height: 13)
                            .overlay {
                                Circle()
                                    .stroke(.compound.bgCanvasDefault, lineWidth: 2)
                            }
                            .accessibilityLabel(room.presence?.presence.accessibilityLabel ?? UntranslatedL10n.screenSettingsPresenceOffline)
                    }
                }
        }
    }
...
struct HomeScreenRoomCellButtonStyle: ButtonStyle {
    let isSelected: Bool
    let palette: SYNThemePalette
    
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .background(isSelected ? Color.compound.bgSubtleSecondary : palette.surface.opacity(palette.isRetro ? 0.96 : 0.90))
            .overlay {
                if palette.isRetro {
            ...
            .clipShape(RoundedRectangle(cornerRadius: palette.isRetro ? palette.cornerRadius : 0))
            .shadow(color: .black.opacity(0.18), radius: palette.shadowRadius, y: 1)
            .padding(.horizontal, palette.isRetro ? 6 : 0)
            .padding(.vertical, palette.isRetro ? 2 : 0)
