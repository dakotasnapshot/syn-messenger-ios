...
class HomeScreenViewModel: HomeScreenViewModelType, HomeScreenViewModelProtocol {
    ...
    private var presenceByUserID = [String: ClientProxyPresenceInfo]()
    private var directUserIDByRoomID = [String: String]()
    private var requestedPresenceUserIDs = Set<String>()
    ...
    init(userSession: UserSessionProtocol,
        ...
        Timer.publish(every: 60, on: .main, in: .common)
            .autoconnect()
            .sink { [weak self] _ in
                self?.refreshDirectRoomPresence()
            }
            .store(in: &cancellables)
    ...
    private func updateRooms() {
        ...
        for summary in roomSummaryProvider.roomListPublisher.value {
            ...
            if let roomID = room.roomID {
                room.directUserID = room.directUserID ?? directUserIDByRoomID[roomID]
            }
            rooms.append(room)
        }
        ...
        let userIDs = Set(rooms.compactMap(\.directUserID))
        if !userIDs.isSubset(of: requestedPresenceUserIDs) {
            refreshDirectRoomPresence()
        }
    }
    
    private func refreshDirectRoomPresence() {
        presenceRefreshTask?.cancel()
        presenceRefreshTask = Task { [weak self] in
            guard let self else { return }
            await resolveMissingDirectUserIDs()
            let userIDs = Set(state.rooms.compactMap(\.directUserID))
            guard !userIDs.isEmpty else { return }
            requestedPresenceUserIDs.formUnion(userIDs)
            
            for userID in userIDs {
                guard !Task.isCancelled else { return }
                if case let .success(presence) = await userSession.clientProxy.presence(for: userID) {
    ...
    private func resolveMissingDirectUserIDs() async {
        let roomIDs = state.rooms.compactMap { $0.isDirect && $0.directUserID == nil ? $0.roomID : nil }
        var resolvedUserIDs = [String: String]()
        
        for roomID in roomIDs {
            guard !Task.isCancelled,
                  case let .joined(roomProxy) = await userSession.clientProxy.roomForIdentifier(roomID),
                  let members = await roomProxy.members(),
                  let member = members.first(where: { $0.isActive && $0.userID != userSession.clientProxy.userID }) else {
                continue
            }
            resolvedUserIDs[roomID] = member.userID
        }
        
        directUserIDByRoomID.merge(resolvedUserIDs) { _, new in new }
        for index in state.rooms.indices {
            guard let roomID = state.rooms[index].roomID else { continue }
            state.rooms[index].directUserID = state.rooms[index].directUserID ?? resolvedUserIDs[roomID]
        }
    }
    
    private func applyCachedPresence() {
        for index in state.rooms.indices {
            guard let userID = state.rooms[index].directUserID else { continue }
            state.rooms[index].presence = presenceByUserID[userID]
        }
    }
