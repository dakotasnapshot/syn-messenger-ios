...
struct RoomDetailsScreen: View {
    ...
    var body: some View {
        ...
        .task { hasWallpaper = SYNRoomWallpaperStore.image(roomID: context.viewState.details.id) != nil }
        .onChange(of: wallpaperItem) { _, item in
            guard let item else { return }
            Task {
                guard let data = try? await item.loadTransferable(type: Data.self),
                      (try? SYNRoomWallpaperStore.save(data, roomID: context.viewState.details.id)) != nil else { return }
                hasWallpaper = true
            }
        }
    }
    ...
    private var configurationSection: some View {
        Section {
            Menu {
                ForEach(SYNRoomWallpaperStore.Preset.allCases) { preset in
                    Button {
                        SYNRoomWallpaperStore.select(preset, roomID: context.viewState.details.id)
                        hasWallpaper = true
                    } label: {
                ...
                Label("Built-in Wallpapers", systemImage: "rectangle.3.group.bubble.left.fill")
                    .foregroundStyle(synTheme.usesMonochromeControls ? .white : synTheme.accent)
            ...
            if hasWallpaper {
                Button("Remove Room Wallpaper", role: .destructive) {
                    try? SYNRoomWallpaperStore.remove(roomID: context.viewState.details.id)
                    hasWallpaper = false
                }
            }
        ...
        .tint(synTheme.usesMonochromeControls ? .white : synTheme.accent)
    }
