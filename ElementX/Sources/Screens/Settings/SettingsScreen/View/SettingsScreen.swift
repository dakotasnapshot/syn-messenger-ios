...
private extension SYNPresenceMode {
    var title: String {
        switch self {
        case .automatic: UntranslatedL10n.screenSettingsPresenceAutomatic
        case .online: UntranslatedL10n.screenSettingsPresenceOnline
        case .unavailable: UntranslatedL10n.screenSettingsPresenceAway
        case .offline: UntranslatedL10n.screenSettingsPresenceOffline
        }
    }
}
...
struct SettingsScreen: View {
    ...
    private var presenceSection: some View {
        Section {
            ListRow(label: .plain(title: UntranslatedL10n.screenSettingsPresenceAvailability),
                    kind: .picker(selection: $context.synPresenceMode,
                                  items: SYNPresenceMode.allCases.map { (title: $0.title, tag: $0) }))
                .onChange(of: context.synPresenceMode) { _, mode in
                    context.send(viewAction: .updatePresenceMode(mode))
                }
            
            ListRow(label: .plain(title: UntranslatedL10n.screenSettingsPresenceShare,
                                  description: UntranslatedL10n.screenSettingsPresenceShareDescription),
                    kind: .toggle($context.sharePresence))
                .onChange(of: context.sharePresence) { _, value in
                    context.send(viewAction: .updateSharePresence(value))
                }
            
            TextField(UntranslatedL10n.screenSettingsPresenceMessage,
                      text: $context.synPresenceStatusMessage)
                .onSubmit {
                    context.send(viewAction: .updatePresenceStatusMessage(context.synPresenceStatusMessage))
                }
        } header: {
            Text(UntranslatedL10n.screenSettingsPresenceTitle)
                .compoundListSectionHeader()
        } footer: {
            Text(UntranslatedL10n.screenSettingsPresenceDescription)
    ...
    private var generalSection: some View {
        Section {
            ...
            ListRow(label: .default(title: L10n.commonReportAProblem,
                                    icon: \.chatProblem),
                    kind: .button {
                        openURL(URL(string: "mailto:support@synmessenger.com?subject=SYN%20Messenger%20Problem%20Report")!) // swiftlint:disable:this force_unwrapping
                    })
