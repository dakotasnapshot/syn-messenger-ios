    ...
    private var roomsNotificationSection: some View {
        Section {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text(UntranslatedL10n.screenNotificationSettingsMentionKeywordsTitle)
                    .font(.compound.bodyMD)
                TextField(UntranslatedL10n.screenNotificationSettingsMentionKeywordsPlaceholder, text: $context.mentionKeywords, axis: .vertical)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                Button(UntranslatedL10n.screenNotificationSettingsMentionKeywordsSave) {
                    context.send(viewAction: .saveMentionKeywords)
                }
                .disabled(context.viewState.applyingChange)
                Text(UntranslatedL10n.screenNotificationSettingsMentionKeywordsDescription)
                    .font(.compound.bodySM)
