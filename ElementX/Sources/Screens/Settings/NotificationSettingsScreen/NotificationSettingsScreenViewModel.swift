...
class NotificationSettingsScreenViewModel: NotificationSettingsScreenViewModelType, NotificationSettingsScreenViewModelProtocol {
    ...
    init(appSettings: AppSettings,
        ...
        let bindings = NotificationSettingsScreenViewStateBindings(enableNotifications: appSettings.enableNotifications,
                                                                   mentionKeywords: appSettings.notificationMentionKeywords.joined(separator: ", "))
    ...
    override func process(viewAction: NotificationSettingsScreenViewAction) {
        ...
        case .saveMentionKeywords:
            Task { await saveMentionKeywords() }
        case .callsChanged:
    ...
    private func saveMentionKeywords() async {
        let keywords = state.bindings.mentionKeywords
            .components(separatedBy: CharacterSet(charactersIn: ",\n"))
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
        do {
            state.applyingChange = true
            try await notificationSettingsProxy.setMentionKeywords(keywords)
            appSettings.notificationMentionKeywords = keywords
            state.bindings.mentionKeywords = keywords.joined(separator: ", ")
        } catch {
            MXLog.error("Failed saving mention keywords: \(error)")
            state.bindings.alertInfo = AlertInfo(id: .alert)
        }
        state.applyingChange = false
    }
