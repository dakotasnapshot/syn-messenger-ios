...
class UserProfileScreenViewModel: UserProfileScreenViewModelType, UserProfileScreenViewModelProtocol {
    ...
    private func fetchPresence() async -> Result<ClientProxyPresenceInfo, ClientProxyError> {
        await userSession.clientProxy.presence(for: state.userID)
    }
    
    private func loadProfile() async {
        async let profileResult = fetchProfile()
        async let identityResult = fetchUserIdentity()
        async let presenceResult = fetchPresence()
        ...
        if case let .success(presence) = await presenceResult {
