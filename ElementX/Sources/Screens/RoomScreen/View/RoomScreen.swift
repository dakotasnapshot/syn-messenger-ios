...
struct RoomScreen: View {
    ...
    var body: some View {
        TimelineView(timelineContext: timelineContext)
            ...
            .background(synTheme.titleBar.ignoresSafeArea())
            ...
            .task { wallpaper = SYNRoomWallpaperStore.image(roomID: context.viewState.roomID) }
            .onReceive(NotificationCenter.default.publisher(for: SYNRoomWallpaperStore.didChange)) { notification in
                guard notification.object as? String == context.viewState.roomID else { return }
                wallpaper = SYNRoomWallpaperStore.image(roomID: context.viewState.roomID)
            }
            .track(screen: .Room)
