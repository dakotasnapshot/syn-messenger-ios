...
class AppCoordinator: AppCoordinatorProtocol, AuthenticationFlowCoordinatorDelegate, NotificationManagerDelegate, SecureWindowManagerDelegate {
    ...
    private let phoneWatchBridge: PhoneWatchBridge
    ...
    private var userSession: UserSessionProtocol? {
        didSet {
            ...
            phoneWatchBridge.configure(userSession: userSession)
            if let userSession {
    ...
    init(appDelegate: AppDelegate) {
        ...
        phoneWatchBridge = PhoneWatchBridge()
    ...
    func toPresentable() -> AnyView {
        AnyView(SYNThemeHost(appSettings: appSettings) {
            navigationRootCoordinator.toPresentable()
        }
        .environment(\.analyticsService, analyticsService)
        .onReceive(appSettings.synColorModePublisher) { [weak self] colorMode in
            ...
            windowManager.windows.forEach { window in
                ...
                window.overrideUserInterfaceStyle = colorMode.interfaceStyle
            }
        })
    }
    ...
    private func handleBackgroundAppRefresh(_ task: BGAppRefreshTask) async {
        ...
        _ = await userSession.clientProxy.configurePresence(.offline, statusMessage: nil, sendImmediately: false)
