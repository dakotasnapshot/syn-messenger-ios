...
class MockScreen: Identifiable {
    ...
    lazy var coordinator: CoordinatorProtocol? = {
        ...
        case .roomPlainNoAvatar:
            ...
            navigationStackCoordinator.setRootCoordinator(coordinator)
            return navigationStackCoordinator
        case .roomSmallTimeline, .appStoreRoomWallpaper:
            let navigationStackCoordinator = NavigationStackCoordinator()
            let timelineItems = id == .appStoreRoomWallpaper ? TimelineFixtures.appStoreConversation : TimelineFixtures.smallChunk
            let timelineController = TimelineControllerMock(.init(timelineItems: timelineItems))
            let roomID = "!weekend-plans:matrix.org"
            if id == .appStoreRoomWallpaper {
                SYNRoomWallpaperStore.select(.connections, roomID: roomID)
            }
            let parameters = RoomScreenCoordinatorParameters(userSession: UserSessionMock(.init()),
                                                             roomProxy: JoinedRoomProxyMock(.init(id: roomID,
                                                                                                  name: id == .appStoreRoomWallpaper ? "Weekend Plans" : "New room",
        ...
        case .userSessionScreen, .userSessionScreenReply, .userSessionSpacesFlow, .appStoreHome:
            let appSettings: AppSettings = appSettings
            ...
            clientProxy.presenceForReturnValue = .success(.init(presence: .online,
...
private extension Array where Element == RoomSummary {
    static let appStoreRooms: [Element] = [
        appStoreRoom(id: "weekend", name: "Weekend Plans", message: "Trailhead at 9 works for me!", isDirect: true, unread: 2),
        appStoreRoom(id: "studio", name: "Studio Crew", message: "The new colors look fantastic.", unread: 0),
        appStoreRoom(id: "maya", name: "Maya Chen", message: "I can bring the picnic blanket.", isDirect: true, unread: 1),
        appStoreRoom(id: "garden", name: "Community Garden", message: "Seed swap is Saturday morning.", unread: 3),
        appStoreRoom(id: "bookclub", name: "Book Club", message: "Next pick: The Long Way Home", unread: 0)
    ]
    
    static func appStoreRoom(id: String, name: String, message: String, isDirect: Bool = false, unread: UInt) -> RoomSummary {
        RoomSummary(room: RoomSDKMock(),
                    ...
                    avatarURL: nil,
                    heroes: [],
                    activeMembersCount: isDirect ? 2 : 8,
                    lastMessage: AttributedString(message),
                    lastMessageDate: .now,
