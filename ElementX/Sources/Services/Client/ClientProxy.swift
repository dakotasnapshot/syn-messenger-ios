...
class ClientProxy: ClientProxyProtocol {
    ...
    func configurePresence(_ presence: ClientProxyPresence, statusMessage: String?, sendImmediately: Bool) async -> Result<Void, ClientProxyError> {
        do {
            if statusMessage != nil {
                try await sendPresence(presence, statusMessage: statusMessage)
            } else {
    ...
    func presence(for userID: String) async -> Result<ClientProxyPresenceInfo, ClientProxyError> {
        do {
            let request = try presenceRequest(userID: userID, method: "GET")
            let (data, response) = try await URLSession.shared.data(for: request)
            try validatePresenceResponse(response)
            let presenceResponse = try JSONDecoder().decode(PresenceResponse.self, from: data)
            return .success(.init(presence: presenceResponse.presence.clientValue,
                                  statusMessage: presenceResponse.statusMessage,
                                  lastActiveAgo: presenceResponse.lastActiveAgo.map { TimeInterval($0) / 1000 },
                                  currentlyActive: presenceResponse.currentlyActive))
        } catch {
            MXLog.error("Failed loading presence for user \(userID): \(error)")
            return .failure(.sdkError(error))
        }
    }
    
    private func sendPresence(_ presence: ClientProxyPresence, statusMessage: String?) async throws {
        var request = try presenceRequest(userID: userID, method: "PUT")
        request.httpBody = try JSONEncoder().encode(PresenceRequest(presence: presence.apiValue,
    ...
    private func presenceRequest(userID: String, method: String) throws -> URLRequest {
        let session = try client.session()
        guard let homeserverURL = URL(string: session.homeserverUrl) else {
            throw ClientProxyError.invalidHomeserverURL
        }
        let url = homeserverURL
            .appendingPathComponent("_matrix/client/v3/presence")
    ...
    private func validatePresenceResponse(_ response: URLResponse) throws {
        guard let response = response as? HTTPURLResponse, 200..<300 ~= response.statusCode else {
            throw ClientProxyError.forbiddenAccess
        }
    }
...
private extension String {
    var clientValue: ClientProxyPresence {
        switch self {
        case "online": .online
        case "unavailable": .unavailable
        default: .offline
